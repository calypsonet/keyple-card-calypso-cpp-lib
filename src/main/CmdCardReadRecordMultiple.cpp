...
const std::map<const int, const std::shared_ptr<StatusProperties>>
    CmdCardReadRecordMultiple::STATUS_TABLE = {
    {
        0x6700,
        ...
        0x6A80,
        std::make_shared<StatusProperties>("Incorrect command data (incorrect Tag, incorrect " \
                                           "Length, R. Length > RecSize, R. Offset + R. Length > " \
                                           "RecSize, R. Length = 0).",
                                           typeid(CardIllegalParameterException))
    }, {
        0x6A82,
        ...
        std::make_shared<StatusProperties>("Successful execution, partial read only: issue " \
                                           "another Read Record Multiple from record (P1 + (Size " \
                                           "of returned data) / (R. Length)) to continue reading.")
    }
};
